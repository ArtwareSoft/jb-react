jb.extension('vscode', {
    ...
    async init() {
        global.spy = jb.spy.initSpy({spyParam: 'dialog,watchable,headless,method,refresh'})
    ...
    createWebViewProvider(id,extensionUri) { 
        ...
        async resolveWebviewView(panel, context, _token) {
            ...
            async function show() {
                console.log(`show ${id}`)
                await id == 'preview' && jb.jbm.terminateChild('wPreview')
                await jb.exec(jbm.child({id: 'wPreview', init: studio.initPreview()}))
                jb.exec({ $: 'vscode.showInWebView', id, panel })
            }
        }
    }},
    api() {
...
jb.component('vscode.logsCtrl', {
    type: 'control',
    impl: studio.eventTracker(()=>jb.spy)
})
