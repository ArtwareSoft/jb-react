...
component('workspace.views', {
  ...
  impl: group({
            //style: group.tabs({selectedTabRef: '%$workspace/bottomViewIndex%'}),
            controls: [
                // text({
                //     title: 'workspace',
                //     text: prettyPrint('%$workspace%'), 
                //     style: text.textarea({rows: 20}),
                //     features: watchRef('%$workspace/probeCircuit%'),
...
component('workspace.openQuickPickMenu', {
    ...
    impl: menu.openContextMenu({menu: '%$menu()%', 
    features: [
...
component('workspace.textEditor', {
  ...
  params: [
    ...
    {id: 'codeMirror', as: 'boolean' },
    {id: 'height', as: 'number', defaultValue: '300'},
  ],
  impl: text({
        text: '%$docContent%',
        style: If('%$codeMirror%', text.codemirror({height: '%$height%'}), text.textarea({rows: 20})),
        features: [
          id('activeEditor'),
          frontEnd.var('docUri','%$docUri%'),
          variable('popupLauncherCanvas','%$cmp%'),
          feature.byCondition('%$codeMirror%',codemirror.initTgpTextEditor(), textarea.initTgpTextEditor('%$selection%')),
          feature.onKey('Ctrl-Enter', action.runBEMethod('onEnter')),

          method('onEnter', ctx => jb.workspace.onEnter(ctx)),
          method('selectionChanged', workspace.selelctionChanged('%%','%$docUri%')), //({data},{},{docUri}) => jb.workspace.selectionChanged(data,docUri)),
          method('contentChanged', (ctx,{},{docUri}) => jb.workspace.contentChanged(ctx.data,docUri,ctx)),
        
          feature.byCondition('%$height%', css.height({height: '%$height%', minMax: 'max'}),null)
        ]
    })
})
