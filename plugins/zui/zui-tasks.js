...
component('baseTask', {
  ...
  params: [
    {id: 'id', as: 'number'},
    {id: 'title', as: 'string'},
    {id: 'noOfItems', as: 'number', options: '1,5,30,50'},
    {id: 'details', as: 'string', options: 'icon,card'},
    {id: 'smartModel', as: 'string', options: ',smart'}
  ]
})
  ...
  impl: (ctx, noOfItemsOptions, detailsOptions, modelOptions) => {
    ...
    return noOfItemsOptions.flatMap(noOfItems =>
      detailsOptions.flatMap(details =>
        modelOptions.flatMap(_model => {
          const model = jb.exec({ $$: `model<llm>${_model}` })
          if (!model) {
            jb.logError(`model id ${_model} is not defined as component`, { ctx })
            return []
          }
          if (details === 'card' && noOfItems > model.maxCards) return []
          const speed = model.speed[details]
          const estimatedDuration = speed[0] * noOfItems + speed[1] // Calculate estimated duration
          const title =
            details === 'contextHints'
              ? `${noOfItems} hints`
              : `more ${noOfItems} ${model.name} items for ${details}`
